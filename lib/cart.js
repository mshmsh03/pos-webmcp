...
export function matchProduct(pool, query, where) {
  ...
  if (candidates.length > 1) {
    ...
    // Name the tool explicitly. An agent that is merely told "be more specific"
    // tends to guess or bounce the question back through chat; told there is a
    // way to ask the person standing at the screen, it uses it.
    throw new Error(
      `"${query}" matches ${candidates.length} products in the ${where}: ${matched}. ` +
        'Do not guess — call ask_cashier with these as the options and let them pick.'
    );
  }
  return candidates[0];
}
...
export function createCartApi({ getProducts, getCart, setCart }) {
  // Declared as closures rather than object methods so nothing depends on
  // `this` — these get passed around and destructured by callers.

  // Add an already-resolved product. The cashier tapping a tile enters here,
  // so both paths share one set of stock and quantity rules — the UI used to
  // enforce only "stock > 0" and let a cashier build a cart that failed at
  // the till, while the agent got the properly validated version.
  ...
  // Resolve a loose name, then delegate. The agent enters here.
  function add(name, quantity) {
    return addProduct(matchProduct(getProducts(), name, 'catalog'), quantity);
  }
  ...
  return { read, add, addProduct, remove, clear };
}
